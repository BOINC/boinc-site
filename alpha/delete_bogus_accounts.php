...
function main() {
    $ids = file("reporters", FILE_IGNORE_NEW_LINES);
    $email_addrs = file("email_list_5_18", FILE_IGNORE_NEW_LINES);
    $query = "delete from user where email_addr not in (";
    foreach ($email_addrs as $a) {
        $query .= "'$a',";
    }
    $query .= "'') and id not in(";
    foreach ($ids as $id) {
