...
function show_group($name, $list, $short_name) {
    ...
    table_header(
        'Add-on<br><font size=-2>click for info</font>',
        'Description'
	);
	shuffle($list);
	foreach ($list as $item) {
        ...
        table_row(
			sprintf(
                '<a href=https://boinc.berkeley.edu/addon_item.php?platform=%s&item=%s>%s</a>',
                $short_name, $iname, $itemname
            ),
            $desc
		);
	}
...
function main() {
    global $win, $linux, $mac, $mobile, $browser, $web, $desktop;
    $is_login_page = true;
    ...
    echo "<a name=desktop></a>\n";
    show_group("Desktop (Windows, MacOS, Linux)", $desktop, 'desktop');
